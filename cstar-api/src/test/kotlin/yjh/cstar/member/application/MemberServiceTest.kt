...
class MemberServiceTest : IntegrationTest() {
    ...
    fun `내정보_조회 테스트`() {
        // given
        val savedMemberId = memberJpaRepository.save(
            MemberEntity(
        ...
        // when
        val member = memberService.retrieveMe(savedMemberId)
    ...
    fun `회원 조회 테스트 - 존재하지 않는 email`() {
        ...
        // then
        assertEquals(HttpStatus.NOT_FOUND, exception.errorCode.httpStatus)
    }
    ...
    fun `회원 생성 시 email이 중복되면 예외를 발생`() {
        ...
        // then
        assertEquals(HttpStatus.CONFLICT, exception.errorCode.httpStatus)
    }
